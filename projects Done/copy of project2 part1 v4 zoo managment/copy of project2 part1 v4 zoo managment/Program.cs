...
namespace ZooManagement
{
    ...
    public class Habitat
    {
        ...
        private Zoo zoo; // object(instance) of class Zoo is created

        public Habitat(string name,Zoo zoo)
        {
            Name = name;
        ...
        {
            Animals.Add(animal);
            zoo.AnimalsInventory.Add(animal.ID, animal);
            Console.WriteLine($"Added {animal.Name} the {animal.Species} to the {Name} habitat with the unique ID:{animal.ID}");
        }
    ...
    public class Zoo
    {
        ...
        public Dictionary<Guid,Animal> AnimalsInventory { get; set; }
        ...
        public void UpdateAnimalCountsByType()
        {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Habitat savannah = new Habitat("Savannah",zoo1);
            Habitat jungle = new Habitat("Jungle",zoo1);
            Habitat pond = new Habitat("Pond",zoo1);
            Habitat mountain = new Habitat("mountain", zoo1);
            ...
            zoo1.AddHabitat(savannah);
            zoo1.AddHabitat(jungle);
            zoo1.AddHabitat(pond);
            zoo1.AddHabitat(mountain);
            ...
            lion = new Lion("limba", 6, savannah);
            Monkey monkey1122 = new Monkey("monkey1122", 5, mountain);


            zoo1.UpdateAnimalCountsByType();
            ...
            zoo1.FeedAllAnimals();
            zoo1.MakeAllAnimalsSound();

            // New line of code to demonstrate getting the count of animals by type
            Console.WriteLine($"\nNumber of Lions in the zoo: {zoo1.GetAnimalCountByType("Lion")}");
            Console.WriteLine($"Number of Elephants in the zoo: {zoo1.GetAnimalCountByType("Elephant")}");
            Console.WriteLine($"Number of Monkeys in the zoo: {zoo1.GetAnimalCountByType("Monkey")}");
            Console.WriteLine($"Number of Fish in the zoo: {zoo1.GetAnimalCountByType("Fish")}");
            ...
            Guid userGuid;
            if (Guid.TryParse(userInput, out userGuid))
            {  if (zoo1.AnimalsInventory.ContainsKey(userGuid))
                   {
                    // Successfully parsed the user input as a Guid
                    Console.WriteLine("User Guid: " + userGuid);
                    Console.WriteLine($"We check the ID : {userGuid} and got :\nID: {zoo1.AnimalsInventory[userGuid].ID}\nName: {zoo1.AnimalsInventory[userGuid].Name}\nAge: {zoo1.AnimalsInventory[userGuid].Age}\nSpecie: {zoo1.AnimalsInventory[userGuid].Species}");
                   }
                else
                {
                    Console.WriteLine("DONOT exist the ID");
                }
               
            }
            else
            {
                // Failed to parse the user input as a Guid
                Console.WriteLine("Invalid Guid input.");
            }
